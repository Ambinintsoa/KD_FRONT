...
@Component({
    selector: 'app-list-historique-rdv',
    standalone: true,
    imports: [CommonModule, DataViewModule, TagModule],
    template: `
    <div class="flex flex-col">
        <div class="card">
            <div class="font-semibold text-xl mb-4">Historique des rendez-vous</div>
...
export class RDVClient implements OnInit {
    ...
    ngOnInit() {
        this.rdvService.getRendezVousClient().subscribe(
            (data) => {
                // Filtrer pour ne garder que les rendez-vous historiques (exemple: statut = 'Terminé')
